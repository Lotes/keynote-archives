...
export function Inspector({ name, url, onUnload }: InspectorProps) {
  ...
  const dictionary = _.groupBy(files, f => f.path.substring(0, f.path.lastIndexOf("/")));
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-start p-24">
      ...
      {progress < maximum && <ProgressBar label="Unpacking..." value={progress} maximum={maximum} />}
      {entries.map(([path, files]) => <div key={path}>
        <h2 className="text-2xl font-bold"><a name={path}></a>{path}</h2>
        {files.map(f => <FileFrame key={f.path} file={f.path}/>)}
      </div>)}
    </main>
  );
}
